...
proc sql;
  ...
  where year(RunDate)  = year(today())
    and month(RunDate) = month(today())
  order by RunDate;
quit;
...
data _null_;
  ...
  put '{"label":"' WeekLabel +(-1) '",'
      ...
      '"ocr":' Current_OCR_Amt ','
      '"cumul":' Cumul_Closed ','
      '"closed_wk":' Closed_This_Week ','
      '"ocr_red":' OCR_Reduced_This_Week ','
      '"pct":' Pct_Claims_Closed
      @;
  if eof then put '}]';
  else put '},';
run;
...
data _null_;
  ...
  /* Escape any quotes in ClaimHandler name */
...
/*--------------------------------------------------------------
  8. Read JSON fragments into macro variables
--------------------------------------------------------------*/
%let tracker_json =;
%let claims_json  =;
...
data _null_;
  ...
  input;
  call symcat('tracker_json', trim(symget('tracker_json')) || trim(_infile_));
run;

data _null_;
  infile "/tmp/claims_json.txt" lrecl=32767 end=eof;
  input;
  call symcat('claims_json', trim(symget('claims_json')) || trim(_infile_));
run;

/*--------------------------------------------------------------
  9. Write the HTML dashboard
     Everything from here to the closing data _null_ is
     pure HTML/CSS/JS — SAS only injects the data variables.
--------------------------------------------------------------*/
data _null_;
  ...
  put "<title>Motor OCR Tracker &mdash; &latest_week. &mdash; &rd.</title>";
  ...
  put '    <p class="eyebrow">FNB ST Insurance &mdash; Motor Retail</p>';
  put "    <h1>OCR Weekly Tracker &mdash; &latest_week.</h1>";
  put '  </div>';
  put '  <div class="header-right">';
  put "    <div><span class='pill'>Motor &bull; Retail &bull; Not Paid Off</span></div>";
  put "    <div style='margin-top:8px;'>Run date: &rd.</div>";
  put "    <div>Baseline: &rd. cycle</div>";
  put '  </div>';
  put '</div>';

  /* Metric cards — values injected from SAS macro vars */
  ...
  put '<button class="toggle-btn" onclick="toggleClaims(this)">Show claims detail &darr;</button>';
  ...
  put '<div class="footer">Generated by OCR_05_Dashboard_Export.sas &bull; FNB ST Analytics &bull; Motor &bull; Retail</div>';
  put '</div>';

  /* ---- Inline JS with SAS-injected data ---- */
  put '<script>';

  /* Inject data from SAS */
  put 'const TRACKER = ';
  put "&tracker_json.;";
  put 'const CLAIMS = ';
  put "&claims_json.;";
  ...
  /* Formatter */
  put 'const fmtR = v => v === 0 ? "&mdash;" : "R " + Number(v).toLocaleString("en-ZA",{minimumFractionDigits:2,maximumFractionDigits:2});';
  ...
  put '    <td>${r.closed_wk === 0 ? "&mdash;" : fmtN(r.closed_wk)}</td>';
  put '    <td>${fmtN(r.cumul)}</td>';
  put '    <td>${r.pct === 0 ? "&mdash;" : fmtP(r.pct)}</td>';
  put '    <td style="font-family:var(--mono)">${fmtR(r.ocr)}</td>';
  put '    <td style="font-family:var(--mono)">${r.ocr_red === 0 ? "&mdash;" : fmtR(r.ocr_red)}</td>';
  put '  `;';
