...
/*--------------------------------------------------------------
  6. Serialize tracker rows to JSON via DATA step
     ...
     with a two-step approach:
       a) Write JSON to a temp file (unchanged).
       b) Read it back using call symput() with cats() to
          concatenate each line into the macro variable.
     Note: macro variables have a 64KB limit. For very large
     claim lists this is safe for the tracker (max ~10 rows)
     but the claims JSON uses a separate file write approach.
--------------------------------------------------------------*/
data _null_;
...
/*--------------------------------------------------------------
  8. Read JSON fragments into macro variables
     FIX 2 (continued): Use call symput() with cats() to
     build each macro variable by appending lines one at a time.
     symput truncates at 32767 chars per call, which is
     sufficient for tracker JSON. Claims JSON is written
     directly to the HTML via %include of the temp file.
--------------------------------------------------------------*/
%let tracker_json = ;
...
/* Claims JSON is large — write it directly into the HTML
   via a separate include rather than a macro variable,
   to avoid the 64KB macro variable size ceiling. */

/*--------------------------------------------------------------
  9. Write the HTML dashboard
     FIX 3: All HTML entities (&mdash; &bull; etc.) replaced
     with their numeric equivalents (&#8212; &#8226;) so SAS
     does not attempt to resolve them as macro variable references.
     Lines that need macro variable injection use double quotes
     only where necessary; purely static HTML lines use single
     quotes to prevent any macro resolution.
--------------------------------------------------------------*/
data _null_;
  ...
  /* Inject tracker data from macro variable (small, safe for symput) */
  put 'const TRACKER = ';
  put "&tracker_json.;";
...
/* Append claims JSON directly from temp file into the HTML output */
data _null_;
