...
/* --- WEEK 3 --- */
%let bf_week3_date       = 2026-04-06;  /* replace if actual run date differs */
%let bf_week3_open       = 999;         /* replace: claims still open */
%let bf_week3_ocr        = 999999.99;   /* replace: total OCR amount */
...
proc sql;
  ...
  execute (
    ...
    VALUES (
      'Week 3',
      CAST('%unquote(&bf_week3_date.)' AS DATE),
      %unquote(&bf_baseline_claims.),
