...
%macro ocr_backfill(
  ...
  %macro _yyyymmdd_to_sasdate(raw, outvar);
    /* Strips spaces, extracts yyyy/mm/dd substrings, calls mdy() */
    data _null_;
      _raw = compress("&raw.");          /* remove all whitespace */
      if lengthn(_raw) = 8 then do;
        _yyyy = input(substr(_raw,1,4), 4.);
        _mm   = input(substr(_raw,5,2), 2.);
        _dd   = input(substr(_raw,7,2), 2.);
        _d    = mdy(_mm, _dd, _yyyy);
      end;
      else _d = .;
      call symputx("&outvar.", _d, 'L');
    run;
